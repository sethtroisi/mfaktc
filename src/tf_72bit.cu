...
__global__ void
#ifndef DEBUG_GPU_MATH
__launch_bounds__(THREADS_PER_BLOCK,2) mfaktc_71(unsigned int exp, int72 k, unsigned int *k_tab, int shiftcount, int144 b, unsigned int *RES, unsigned int *PROOF)
#else
__launch_bounds__(THREADS_PER_BLOCK,2) mfaktc_71(unsigned int exp, int72 k, unsigned int *k_tab, int shiftcount, int144 b, unsigned int *RES, unsigned int *PROOF, unsigned int *modbasecase_debug)
#endif
/*
...
/* finally check if we found a factor and write the factor to RES[] */
  ...
  /* TODO proof for 71_bit kernel, can reuse save_proof/check_factor96 code. */
}
